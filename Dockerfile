FROM ubuntu:latest
RUN apt-get update ;\
    DEBIAN_FRONTEND=noninteractive apt-get -o APT::Get::AllowUnauthenticated=true install -yqq gpg wget ;\
    ...
    apt-get update ;\
    DEBIAN_FRONTEND=noninteractive apt-get -o APT::Get::AllowUnauthenticated=true install -yqq closure ;\
